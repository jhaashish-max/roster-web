...
export default function LoginPage() {
    ...
    const handleGoogleLogin = async () => {
        ...
        setError('');
        try {
            await signInWithGoogle(); // browser redirects; loading stays on
        } catch (err) {
    ...
    return (
        <div className="login-page">
            ...
            <div className="login-card card">
                <div className="login-logo"><Logo height="80px" /></div>
                {error && (
                ...
                <button type="button" className="btn btn-primary btn-lg login-btn" onClick={handleGoogleLogin} disabled={loading}>
                    {loading ? (
                        <><Loader2 size={16} className="spin" aria-hidden="true" /> Redirecting to Google…</>
                    ) : (
                        <>
                ...
                <p className="login-hint">Use your Razorpay Google account</p>
            </div>
        </div>
    );
}
